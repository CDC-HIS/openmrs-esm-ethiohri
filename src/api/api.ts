...
export function getValidPatientFollowUpDate(
  patientUuid: string,
  conceptUuid: string,
  answerUuid: string,
  encounterUuid: string
) {
  const url = `${restBaseUrl}/ethiohri/validatencounter?patientUuid=${patientUuid}&encounterTypeUuid=${encounterUuid}&conceptUuid=${conceptUuid}&obsDate=${"2025-06-19"}`;
